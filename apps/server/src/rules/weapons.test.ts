...
describe("weapon masteries (SRD 5.2.1)", () => {
  ...
  it("assigns the SRD mastery to each melee weapon", () => {
    ...
    expect(byName.get("Light Hammer")).toBe("nick");
    ...
    expect(byName.get("Glaive")).toBe("graze");
    ...
    expect(byName.get("Morningstar")).toBe("sap");
    ...
    expect(byName.get("War Pick")).toBe("sap");
    expect(byName.get("Warhammer")).toBe("push");
    expect(byName.get("Whip")).toBe("slow");
  });

  it("assigns the SRD mastery to each ranged weapon", () => {
    const byName = new Map(WEAPONS.map((w) => [w.name, w.mastery]));
    expect(byName.get("Light Crossbow")).toBe("slow");
    expect(byName.get("Dart")).toBe("vex");
    expect(byName.get("Shortbow")).toBe("vex");
    expect(byName.get("Sling")).toBe("slow");
    expect(byName.get("Blowgun")).toBe("vex");
    expect(byName.get("Hand Crossbow")).toBe("vex");
    expect(byName.get("Heavy Crossbow")).toBe("push");
    expect(byName.get("Longbow")).toBe("slow");
  });

  it("covers every weapon with exactly one known mastery", () => {
    for (const weapon of WEAPONS) {
